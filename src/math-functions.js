...
export function sumAndMultiplyThreeNumbers(a, b, c) { //eslint-disable-line
    ...
    let totalProduct = multiply(mul[0], c);
    let sumStr = `${a} and ${b} and ${c} sum to ${totalSum[0]}.`;
    let prodStr = `The product of ${a} and ${b} and ${c} is ${totalProduct[0]}.`;
     
    
    return [totalSum[0], totalProduct[0], sumStr, prodStr];
}
...
export function sumArrayWithThreeNumbers(sumArr) {
    let num = sum(sumArr[0], sumArr[1]);
    let totalSum = sum(num[0], sumArr[2]);
    return [totalSum[0], `${sumArr[0]},${sumArr[1]},${sumArr[2]} was passed in as an array of numbers, and ${totalSum[0]} is their sum.`];
}
...
export function multiplyArrayWithThreeNumbers(multArr) { //eslint-disable-line
    let num = multiply(multArr[0], multArr[1]);
    let totalMult = multiply(num[0], multArr[2]);
    return [totalMult[0], `The numbers ${multArr[0]},${multArr[1]},${multArr[2]} have a product of ${totalMult[0]}.`];
}
...
export function multiplyAnyArray(dynamicArray) { //eslint-disable-line
    let dynamicArrMult = 1;
    for(let product of dynamicArray) {
        dynamicArrMult = multiply(dynamicArrMult, product)[0];
    }
    return [dynamicArrMult, `The numbers ${dynamicArray} have a product of ${dynamicArrMult}.`]
}
